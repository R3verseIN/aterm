...
export function buildAgentPrompt(opts: {
  ...
  return [
    ...
    `- Screenshot is fresh-only: every GET emits request-screenshot:{id} and holds up to 10s via wait_for_screenshot(10). No stale cache. 503 means timeout — read logs.frontend_last_error.`,
    ...
    `- POST ${baseUrl}/input  body {"data":"ls -la\\n"}  (\\n or \\r = Enter, \\x03 = Ctrl-C, \\x04 = Ctrl-D). Must send valid JSON — escape via jq -n --arg.`,
    ...
    `3. POST ${baseUrl}/input  with jq-escaped JSON, check {"ok":true}. Sleep 0.6s.`,
    `4. Poll GET ${baseUrl}/output until idle: stable 3x identical stripped data (sleep 0.5, max 6 loops). Large output: compare total growth instead.`,
    `   prev=""; stable=0; for i in 1 2 3 4 5 6; do cur=$(curl -sSf ${baseUrl}/output | jq -r .data | sed -E 's/\\x1B\\[[0-9;]*[a-zA-Z]//g'); [ "$cur" = "$prev" ] && stable=$((stable+1)) || stable=0; prev="$cur"; [ $stable -ge 2 ] && break; sleep 0.5; done`,
    `5. GET ${baseUrl}/screenshot --max-time 15 for visual. On 503 read logs. For vision use ?format=base64 then cut -d, -f2 | base64 -d.`,
    ...
    `sleep 0.7`,
    `# poll dump-all until idle, strip ANSI, show tail`,
    `prev=""; stable=0; for i in 1 2 3 4 5 6; do cur=$(curl -sSf "$BASE/output" | jq -r .data | sed -E 's/\\x1B\\[[0-9;]*[a-zA-Z]//g; s/\\x1B\\]0;.*\\x07//g'); [ "$cur" = "$prev" ] && stable=$((stable+1)) || stable=0; prev="$cur"; echo "---poll $i stable $stable total $(curl -s "$BASE/output" | jq .total)---"; echo "$cur" | tail -n 100; [ $stable -ge 2 ] && break; sleep 0.6; done`,
