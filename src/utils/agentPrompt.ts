...
export function buildAgentPrompt(opts: {
  ...
  return [
    ...
    `- Simple dump-all: GET /output returns the whole 512KB ring (no cursor). Poll every 500-1000ms and compare. POST /clear wipes it.`,
    ...
    `- GET ${baseUrl}/output -> {"data","total","id"}  whole buffer (max 512KB), simple dump — no since/next_offset`,
    `- POST ${baseUrl}/input  body {"data":"ls -la\\n"}  (\\n or \\r = Enter, \\x03 = Ctrl-C)`,
    `- POST ${baseUrl}/clear -> {"ok":true,"cleared":true,"id"}  clears OUTPUTS ring + screenshot waiters (no old logs)`,
    ...
    `WORKFLOW`,
    `1. GET ${baseUrl}/output to see current screen (text).`,
    `2. POST ${baseUrl}/input to run a command.`,
    `3. Poll ${baseUrl}/output until idle (diff previous data).`,
    `4. GET ${baseUrl}/screenshot for visual verification (even if tab not focused — port is that tab's screenshot).`,
    `5. Repeat. Call POST /clear when you want a fresh buffer.`,

    `EXAMPLE`,
    `curl -s ${baseUrl}/output | jq -r .data`,
    `curl -s -X POST -H "Content-Type: application/json" -d '{"data":"echo hi\\n"}' ${baseUrl}/input`,
    `curl -s "${baseUrl}/output" | jq .`,
